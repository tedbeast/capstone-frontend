...
export function ResetPassword (props: { data: { employeeId: any; }; }) {
    ...
    const resetSubmit = async (event:React.FormEvent<HTMLFormElement>) => {
        ...
        if(pwdSuccess === 400) {
            return setAlertMessage("New password cannot be the same as the current password.")
        }
        if (pwdSuccess === 200) {
            setShowForm(false);
            setShowSuccess(true);
    ...
    return (
        <div>
            ...
            {showSuccess && (
                <><div style={{fontSize: 20, paddingTop: 10, paddingBottom: 5}}>Password successfully updated!</div>
                <div style={{paddingBottom: 10}}>Please return to the Login Page and log in.</div>
